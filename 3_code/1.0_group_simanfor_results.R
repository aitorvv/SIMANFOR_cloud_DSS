...
# set directory
general_dir <- "*/2_simanfor/output/english/"
setwd(general_dir)
...
# total carbon
df$CARBON <- df$WT*ppin_carbon

# stem carbon
df$CARBON_s <- df$WSW*ppin_carbon

# branches carbon
df$CARBON_b <- df$WB*ppin_carbon

# roots carbon
df$CARBON_r <- df$WR*ppin_carbon
...
# round ages and adjust them by below to avoid noise on the graphs
new_df$T <- redondeo(new_df$T - 2, 5) 

# get scenario code
new_df$n_scnr <- substr(new_df$Scenario_file_name, 6, 6)
...
save.image('*/1_data/1_results/simulation_results.RData')
